...
const config = {
  ...
  function create() {
    ...
    socket.on("updatePlayers", (serverPlayers) => {
        ...
        for (let id in players) {
            if (id === socket.id) {
                document.getElementById("player-hp").innerText = `HP: ${players[id].health}`;
            }
        }
    });
    ...
    wallGroup
      .create(1100, 600, "wall")
      .setDisplaySize(50, 500)
      .refreshBody();
    wallGroup
      .create(200, 300, "wall")
      .setDisplaySize(500, 50)
      .refreshBody();
  ...
  function update() {
    ...
    let dx = 0, dy = 0;
    if (cursors.left.isDown) {dx = -5;playerSide="right"}
    if (cursors.right.isDown) {dx = 5;playerSide="left"};
    ...
    for (let id in players) {
        ...
        if (!playersContainer[id]) {
            ...
            playersContainer[id] = this.add.sprite(p.x, p.y, "player").setFrame(0).setScale(0.8);
        } else {
    ...
    bulletList.forEach(b => {
      ...
      if(!bulletContainer[id]){
        bulletContainer[id] = this.physics.add.sprite(b.x, b.y, 'bullet').setScale(0.1);
      }else{
